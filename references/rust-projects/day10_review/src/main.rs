...
// ============================================================================
fn main() {
    println!("Day 10: Comprehensive Review & Practice");
    println!("========================================\n");
    ...
    println!("   three_sum([-1,0,1,2,-1,-4]) = {:?}", result);
    
    // Max Area
    println!("\n3. Container With Most Water");
    let result = max_area(&[1, 8, 6, 2, 5, 4, 8, 3, 7]);
    println!("   max_area([1,8,6,2,5,4,8,3,7]) = {}", result);
    ...
    println!("   group_anagrams(...) = {} groups", result.len());
    
    // Longest Substring
    println!("\n5. Longest Substring Without Repeating");
    let result = length_of_longest_substring("abcabcbb");
    println!("   length_of_longest_substring(\"abcabcbb\") = {}", result);
    
    // Palindromic Substrings
    println!("\n6. Count Palindromic Substrings");
    let result = count_palindromic_substrings("abc");
    println!("   count_palindromic_substrings(\"abc\") = {}", result);
    ...
    min_stack.push(0);
    min_stack.push(-3);
    println!("   After pushing -2, 0, -3: min = {:?}", min_stack.get_min());
    min_stack.pop();
    println!("   After pop: top = {:?}, min = {:?}", min_stack.top(), min_stack.get_min());
    
    // Daily Temperatures
    println!("\n8. Daily Temperatures");
    let temps = vec![73, 74, 75, 71, 69, 72, 76, 73];
    let result = daily_temperatures(&temps);
    println!("   daily_temperatures([73,74,75,71,69,72,76,73]) = {:?}", result);
    ...
    println!("   reverse(1->2->3) = {}->{}->{}", 
             reversed.as_ref().unwrap().val,
             reversed.as_ref().unwrap().next.as_ref().unwrap().val,
             reversed.as_ref().unwrap().next.as_ref().unwrap().next.as_ref().unwrap().val);
    ...
    println!("   max_depth(tree) = {}", max_depth(root));
    
    // Validate BST
    println!("\n11. Validate BST");
    let bst = Some(Box::new(TreeNode {
        val: 2,
        left: Some(Box::new(TreeNode::new(1))),
        right: Some(Box::new(TreeNode::new(3))),
    }));
    println!("   is_valid_bst(tree) = {}", is_valid_bst(bst));
    ...
    println!("   bfs(graph, start=2) = {:?}", result);
    ...
    println!("   dijkstra(graph, start=0) = {:?}", distances);
