...
impl BloomFilter {
    ...
    fn hash(&self, item: &str, seed: usize) -> usize {
        ...
        for byte in item.as_bytes() {
            result = (result.wrapping_mul(seed as u32) as usize).wrapping_add(*byte as usize);
        }
        result % self.size
    }
...
impl SkipList {
    ...
    fn insert(&mut self, val: i32) {
        ...
        for i in (1..=self.level).rev() {
            ...
            update[i] = current as *const SkipListNode;
        }
        ...
        if current_next.is_none() || current_next.unwrap().val != val {
            ...
            if new_level > self.level {
                for i in self.level + 1..=new_level {
                    update[i] = &self.head as *const SkipListNode;
                }
            ...
            for i in 1..=new_level {
                let update_node = unsafe { &mut *(update[i] as *mut SkipListNode) };
...
impl AVLTree {
    ...
    fn right_rotate(&self, y: Box<AVLNode>) -> Box<AVLNode> {
        ...
        x.right = Some(y);
        let mut y_mut = x.right.as_mut().unwrap();
        y_mut.left = t2;
    ...
    fn left_rotate(&self, x: Box<AVLNode>) -> Box<AVLNode> {
        ...
        y.left = Some(x);
        let mut x_mut = y.left.as_mut().unwrap();
        x_mut.right = t2;
    ...
    fn _insert(&self, node: Option<Box<AVLNode>>, key: i32) -> Option<Box<AVLNode>> {
        ...
        let mut node_mut = node.as_mut().unwrap();
        if key < node_mut.key {
        ...
        let balance = self.balance_factor(&Some(node_mut.clone()));
